...
trigger DEL_CaseCommentTrigger on DEL_CaseComment__c (after insert) {
    /* Skip this trigger when DEL_CaseCommentTriggerHelper.blnSkipTrigger is true
       ...
       CMDEL0004 is the name of the record used for this trigger.
    */
    if (
        (DEL_TriggerConfiguration__mdt.getInstance('CMDEL0004') != null &&
         !DEL_TriggerConfiguration__mdt.getInstance('CMDEL0004').IsActive__c) ||
